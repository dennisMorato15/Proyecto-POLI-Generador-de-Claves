            ...
            steps {
                script {
                    ...
                    sh '''
                        ...
                        git clone https://github.com/dennisMorato15/Proyecto-POLI-Generador-de-Claves.git .
                        ls -la
                        git log --oneline -1
                    '''
                    env.GIT_COMMIT_SHORT = sh(
                        script: 'cd /var/jenkins_home/workspace/CI-Generador-Claves && git rev-parse --short HEAD',
                        returnStdout: true
                    ).trim()
